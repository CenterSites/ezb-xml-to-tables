...
//UNZIP ZIPFILE
//TRANSFORM UNZIPPED XML WITH XSL
//DELETE ZIPFILE
//TRUNCATE: productsImport
//INSERT XML into empty table: productsImport

//CREATE CSV file from productsImport (is extra)
//COMPARE all records and columns with latest tables: productsImport > productsHistory
//DROP table: productMutations
//CREATE new table: productMutations
//INSERT unmatched records into: productMutations from latest table: productsImport
//REPLACE INTO voeg records toe en vervang aan table: productsUpsert uit tabel: productMutations
...
//Benamingen database tabellen vanuit formulier
	if (isset($_POST['tableSuffix'])) { //taalcodes
			$tableSuffix = $_POST['tableSuffix'];
	}else{  
			$tableSuffix = "";
	}

	if (!empty($_POST['tablePrefix'])) { //custom tabelnaam
			$tablePrefix = "CSTM_".$_POST['tablePrefix']."_";
	}else{  
			$tablePrefix = "";
	}
...
//Kopieer laatste XML export table productsImport naar nieuwe productsHistory tabel OUDE WERKWIJZE
//createNewHistoryTable($host,$db,$user,$pass,$productsHistory, $productsImport);



	if (!empty($_POST['zip'])) {
			$zipToTR = $_POST['zip'];
			$xmlToDB = str_replace("zip","xml", $_POST['zip']); //geef getransformeerde xml dezelfde naam als de bron zip file
	}else{  
			$zipToTR = "mag_NL.zip";
...
$sqlClear = "DROP TABLE IF EXISTS $db.$productsImport";
	if(mysqli_query($mysqli, $sqlClear)){
	    echo ($debug == 'on' ? "3A) Vorige tabel $db / $productsImport is verwijderd.<br>" : "");
	} else{
	    echo ($debug == 'on' ? "ERROR: Could not able to execute $sqlClear. <br>" . mysqli_error($mysqli) : "");
}
...
$sqlCreate = "CREATE TABLE $db.$productsImport LIKE $db.productsStructure";
	if(mysqli_query($mysqli, $sqlCreate)){
	    echo ($debug == 'on' ? "3B) Nieuwe lege tabel $db / $productsImport is gemaakt obv tabel productsStructure <br>" : "");
	} else{
	    echo ($debug == 'on' ? "ERROR: Could not able to execute $sqlCreate. <br>" . mysqli_error($mysqli) : "");
}
...
$xml = simplexml_load_file("xml/".$xmlToDB) or die("Error: Cannot create object");
    ...
    foreach($articleRecords as $record) {
				...
				//print_r(getAssetsExt($record,'Url','Icon','GRP', 'huvema-picto-')."<br>");	
    }
...
//debuggen
echo ($debug == 'on' ? "4) ".$rows . " records toegevoegd aan ".$db.", table ".$productsImport."<br>" : "");    



// CSVFile maken
$csvfilename = str_replace("zip","csv", $zipToTR);
$sql = "SELECT * FROM $db.$productsImport";

	$result = $mysqli->query($sql);
	if (!$result) {echo "SQL Error<br>";}

	$f = fopen("csv/".$csvfilename, 'w');
	if (!$f) {echo "Could not open file!<br>";}

		// Add column headers
		//array_unshift($dataColumns , 'id'); //add autoincrement ID column
	    fputcsv($f, $dataColumns);
...
//opruimen
fclose($f);
echo ($debug == 'on' ? "5) CSV bestand ".$csvfilename." is aangemaakt. <br>" : "");
mysqli_close($mysqli);
echo ($debug == 'on' ? "6) Database verbinding gesloten <br><br>" : "");
echo '<a href="http://huvemanode.hypernode.io/ezbase/csv/'.$csvfilename.'">Download CSV bestand: '.$csvfilename.'</a><br><br>';
echo '<a href="http://huvemanode.hypernode.io/ezbase">Terug</a><br><br>';
